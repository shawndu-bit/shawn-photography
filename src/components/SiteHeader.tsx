...
export default function SiteHeader({ mode }: SiteHeaderProps) {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${scrolled || mobileOpen ? 'bg-black/55 backdrop-blur-xl' : 'bg-transparent'}`}>
      <div className="mx-auto flex h-16 max-w-[1400px] items-center justify-between px-5 md:px-8 lg:px-12">
        {mode === 'home' ? (
          <a href="#home" aria-label="Shawn Photography 首页" className="shrink-0" onClick={() => setMobileOpen(false)}>
            <img src="/logo.png" alt="Shawn Photography Logo" loading="lazy" className="h-10 w-auto object-contain md:h-12" />
          </a>
        ) : (
          ...
          <ul className="flex h-full items-center justify-center gap-4 text-sm font-medium uppercase tracking-[0.18em] text-zinc-100 md:gap-6 md:text-base">
            {mode === 'home' ? (
              <>
                ...
                <li><Link to="/gallery" className={navItemClass}>Gallery</Link></li>
        ...
        <button type="button" className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/15 text-white/80 transition hover:border-white/40 hover:text-white md:hidden" onClick={() => setMobileOpen((v) => !v)} aria-expanded={mobileOpen} aria-label={mobileOpen ? 'Close menu' : 'Open menu'}>
          ...
          <ul className="flex flex-col gap-4">
            {mode === 'home' ? (
              <>
                ...
                <li><Link to="/gallery" className={navItemClass} onClick={() => setMobileOpen(false)}>Gallery</Link></li>
